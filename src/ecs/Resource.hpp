...
template <typename Content> class cevy::ecs::Resource {
  private:
  Content &_content;
...
template <class T> struct is_resource : public std::false_type {};

template <typename T> struct is_resource<cevy::ecs::Resource<T>> : public std::true_type {};
...
class ResourceManager {
  ...
  template <typename Content> void insert_resource(const Content &value) {
    auto a = std::make_any<Content>(value);

    _resources_map[std::type_index(typeid(Content))] = a;
  }

  template <typename Content> std::optional<Content> remove_resource() {
  ...
  template <typename Content> Content &resource() {
    return std::any_cast<Content &>(_resources_map[std::type_index(typeid(Content))]);
  }

  template <typename Content> const Content &resource() {
    return std::any_cast<const Content &>(_resources_map[std::type_index(typeid(Content))]);
  }

  template <typename Content> std::optional<std::reference_wrapper<Content>> get_resource() {
    auto it = _resources_map.find(std::type_index(typeid(Content)));
  ...
  */
  template <typename Content> cevy::ecs::Resource<Content> get() {
    return cevy::ecs::Resource(
        std::any_cast<Content &>(_resources_map[std::type_index(typeid(Content))]));
  }

  template <typename Content> bool contains_resource() {
