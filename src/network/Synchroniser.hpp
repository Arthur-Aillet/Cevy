...
class cevy::Synchroniser : virtual public cevy::ecs::Plugin {
    public:
        ...
        void add_sync(cevy::ecs::App& app) {
            ...
            app.add_class_system<F, SyncStage, cevy::ecs::Query<SyncId, Component...>>(F(mode, *this, _net));
            // ([&] {
        ...
        void add_spawnable(uint8_t i, T... args) {
            _spawnCommands[i] = [args...](ecs::EntityCommands e) {
                e.insert(T(args)...);
            };
        }
    ...
    protected:
        void build(cevy::ecs::App& app) override {
            ...
            app.add_class_system<SyncSubSystem, SummonStage, cevy::ecs::Commands, cevy::ecs::Query<SyncId>>(std::move(sys));
            build_custom(app);
        }
};
