...
void cevy::physics::Grid::addEntity(const size_t x,const size_t y,const size_t z, const  cevy::physics::entity& value) {
        if (x >= _cells.size()) {
            _cells.resize(x + 1);
...
void cevy::physics::Grid::setGrid(ecs::World& world) {
    ecs::Query<cevy::physics::Collider, option<cevy::engine::Position>, option<cevy::engine::Rotation>> query =
    ecs::Query<cevy::physics::Collider, option<cevy::engine::Position>, option<cevy::engine::Rotation>>::query(world);
    ...
    for (int dx = -1; dx <= 1; dx++) {
        for (int dy = -1; dy <= 1; dy++) {
            for (int dz = -1; dz <= 1; dz++) {
                int x = std::get<const cevy::engine::Position&>(entity1).x / cevy::physics::CELL_SIZE::X + dx;
                int y = std::get<const cevy::engine::Position&>(entity1).y / cevy::physics::CELL_SIZE::Y + dy;
                int z = std::get<const cevy::engine::Position&>(entity1).z / cevy::physics::CELL_SIZE::Z + dz;
                if (x >= 0 && y >= 0 && z >= 0 && x < static_cast<int>(_cells.size()) && y < static_cast<int>(_cells[0].value().size()) && z < static_cast<int>(_cells[0].value()[0].value().size())) {
                    for (auto& entity2 : _cells[x].value()[y].value()[z].value()) {
                        if (std::get<const cevy::physics::Collider&>(entity1).getShape().calculateCollision(std::get<const cevy::physics::Collider&>(entity2).getShape())) {
                            std::cout << "entity 1 in position " << x << ", " << y << ", " << z;
                            std::cout << " have a collision with the entity in position " << std::get<const cevy::engine::Position&>(entity2).x << ", " << std::get<const cevy::engine::Position&>(entity2).y << ", " << std::get<const cevy::engine::Position&>(entity2).z << std::endl;
                        }
                    }
                }
            }
